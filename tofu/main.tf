...
resource "proxmox_virtual_environment_download_file" "ubuntu_cloud_image_nfs" {
  provider     = bpg
  content_type = "iso"
  datastore_id = "local" # The datastore where you want to keep the downloaded image
...
resource "proxmox_virtual_environment_vm" "nfs_vm" {
  ...
  disk {
    datastore_id = "local-lvm" # Where the actual VM disk will live
    file_id      = proxmox_virtual_environment_download_file.ubuntu_cloud_image_nfs.id
    interface    = "virtio0"
    iothread     = true
    discard      = "on"
    size         = 50 # 50GB for OS and some NFS storage, or maybe a separate disk?
  }
